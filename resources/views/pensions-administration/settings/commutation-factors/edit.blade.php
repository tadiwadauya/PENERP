...
@section('title', 'Edit Commutation Factor')

@section('page-heading', 'Edit Commutation Factor')

@section('page-subheading')
Edit a future commutation factor before it takes effect
@endsection
    ...
    <div class="card-body">
        <div class="row align-items-center">
            <div class="col-lg-8">
                ...
                <p class="text-muted mb-0">This factor can be changed because it has not yet taken effect.</p>
            </div>
...
<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('pensions-administration.settings.commutation-factors.update', $factor) }}">
            @csrf
            @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="age_months" class="form-select @error('age_months') is-invalid @enderror" required>
                            @for($month = 0; $month <= 11; $month++)
                                <option value="{{ $month }}" @selected((string) old('age_months', $factor->age_months) === (string) $month)>{{ $month }} {{ \Illuminate\Support\Str::plural('month', $month) }}</option>
                            @endfor
                        </select>
