...
class BenefitSettingsDashboardController extends Controller
{
    ...
    {
        $this->ensurePermission('pensions.settings.view');

        $counts = [
            'general_settings' => BenefitSetting::query()->count(),
            'withdrawal_scales' => WithdrawalEmployerEntitlementScale::query()->count(),
            'interest_rates' => InterestRate::query()->count(),
            'accumulated_interest_factors' => AccumulatedInterestFactor::query()->count(),
            'commutation_factors' => CommutationFactor::query()->count(),
            'retirement_increase_factors' => RetirementAgeIncreaseFactor::query()->count(),
            'tax_tables' => BenefitTaxTable::query()->count(),
            'exchange_rates' => ExchangeRate::query()->count(),
        ];

        return view('pensions-administration.settings.index', compact('counts'));
    }
    ...
    */
    ...
    private function ensurePermission(string $permission): void
    {
        ...
        abort_unless(
            $user->hasRole('system-administrator') && $user->can($permission),
            403,
